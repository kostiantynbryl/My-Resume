...
const internalText=['Portfolio infrastructure','SYSTEM / DELIVERY','BUILD / LOCAL SNAPSHOT','What ships / recent activity','Что выпускается / последняя активность'];
...
async function forbidInternal(page,lang,label,body){for(const s of internalText)if(body.includes(s))throw new Error(`${lang} ${label} leaked internal copy: ${s}`);for(const sel of internalSelectors){if(await page.locator(sel).count())throw new Error(`${lang} ${label} leaked internal widget: ${sel}`)}}
...
for(const [lang,c] of Object.entries(checks)){
  ...
  await forbidInternal(page,lang,'home',home);
  ...
  await forbidInternal(page,lang,'ClearUp',project);
  ...
  await forbidInternal(page,lang,'work',work);

  const about=await bodyFor(page,'/about/',1500);
  await forbidInternal(page,lang,'about',about);

  const now=await bodyFor(page,'/now/',1500);
  await forbidInternal(page,lang,'now',now);
  ...
  await forbidInternal(page,lang,'contact',contact);
