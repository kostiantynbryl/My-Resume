(()=>{
  const A=window.KB_APP,I=window.KB_I18N,H=window.KB_HEADER,M=window.KB_MAIN,F=window.KB_FOOTER,U=window.KB_UTILS;if(!A||!I||!H||!M||!F)return;
  ...
  function render(){H.render();M.render();F.render();U?.render();title();document.body.classList.add('kb-ready')}
  render();
